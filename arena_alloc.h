...
#define AUTO_DEALLOCATE(Type, ptr) \
  if (ptr != nullptr) { \
    ptr->~Type(); \
    engineArena.deallocate((std::byte*)ptr, 1); \
    ptr = nullptr; \
  }

#define AUTO_DEALLOCATE_CA(engineArena, Type, ptr) \
  if (ptr != nullptr) { \
    ptr->~Type(); \
    engineArena.deallocate((std::byte*)ptr, 1); \
    ptr = nullptr; \
...
struct Allocator
{
  ...
  inline std::size_t used_bytes() const { return detail::used_bytes(); }
  inline std::size_t total_bytes() const { return detail::total_bytes(); }
...
inline T* arena_allocate(const arena::Allocator<std::byte>& allocator, Args&&... args) {
  void* mem = const_cast<arena::Allocator<std::byte>&>(allocator).allocate(sizeof(T));
